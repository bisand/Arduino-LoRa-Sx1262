...
{
  _pin_dio1 = SX1262_DIO1;
  _pin_nss = SX1262_NSS;
  _pin_reset = SX1262_RESET;
}

LoraSx1262::LoraSx1262(uint8_t dio1, uint8_t nss, uint8_t reset)
{
  _pin_dio1 = dio1;
  _pin_nss = nss;
  _pin_reset = reset;
...
/*Set the power level of the radio.  This is how much power the radio will use to transmit a packet.
* Higher power levels will transmit further, but will also use more power.
* The SX1262 has 22 power levels, ranging from 0-22.  0 is the lowest power, 22 is the highest.
* The default power level is 22.
*
* Returns TRUE on success, FALSE on failure (invalid power level)
*/
bool LoraSx1262::configSetPowerLevel(uint8_t powerLevel) {
  //Power level must be 0-22
  if (powerLevel < 0 || powerLevel > 22) { return false; }

  //Set power level
  digitalWrite(_pin_nss,0);     //Enable radio chip-select
  spiBuff[0] = REG_TX_POWER;    //Opcode for SetTxParams
  spiBuff[1] = powerLevel;      //Power level
  spiBuff[2] = 0x02;            // Use ramp-up time of 200us
  SPI.transfer(spiBuff,3);
  digitalWrite(_pin_nss,1);     //Disable radio chip-select
  waitForRadioCommandCompletion(100);  //Give time for radio to process the command
  return true;
...
/*Set the sync word for the radio.  This is a special word that the radio uses to know when a packet is starting.
* This is useful when you have multiple radios in the same area, and you want to make sure that a packet is intended for your radio.
* The default sync word is 0x12.  This is a good default, but you can change it if you want.
*
* Returns TRUE on success, FALSE on failure (invalid sync word)
*/
bool LoraSx1262::configSetSyncWord(uint8_t syncWord) {
  //Set Sync Word
  digitalWrite(_pin_nss,0); //Enable radio chip-select
  spiBuff[0] = REG_SYNC_WORD;          //Opcode for "SetSyncWord"
  spiBuff[1] = syncWord;      //Sync word
  SPI.transfer(spiBuff,2);
  digitalWrite(_pin_nss,1); //Disable radio chip-select
  waitForRadioCommandCompletion(100);  //Give time for radio to process the command
  return true;  
